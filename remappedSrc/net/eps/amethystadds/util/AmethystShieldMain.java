...
public class AmethystShieldMain {
    ...
    public static void damage(Entity entity, DamageSource source) {
        if ((entity instanceof PlayerEntity player) && player.getActiveItem().getItem() instanceof AmethystShield) {
...
//                ProjectileEntity projectile = (ProjectileEntity) sourceEntity;
                if (chance < 90) {
                    AmethystShield.decreaseAmethystCount(stack);
                }
            }

            if ((source.getSource() instanceof LivingEntity) && AmethystShield.getAmethystsCount(stack) >= 1) {
                sourceEntity.damage(DamageSource.player(player), damage);
